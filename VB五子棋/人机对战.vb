...
Public Class 人机对战
    ...
    Private Function score(ByVal m As Integer, ByVal n As Integer, ByVal k As Integer) As Integer   '计分函数（棋盘横坐标，纵坐标，棋子标签）
        ...
        While i < 19 And i < m + 5 And j < 19              '左斜方向
            If Form1.qipan(i, j) <> s Then
                ...
                Exit While
            Else
                p = p + 1   '同样 先向右下再向右左上， 左右的分数加起来，即为此点在左斜方向上的分数
            End If
            i = i + 1
            j = j + 1
        End While
        ...
        While i > 0 And i > m - 5 And j < 19                          '  右斜部分
            If Form1.qipan(i, j) <> s Then
                ...
                Exit While
            Else
                p = p + 1   '同样 先向左下再向右上， 左右的分数加起来，即为此点在右斜方向上的分数
            End If
            i = i - 1
            j = j + 1
        End While
        ...
        Else
            i = 0
            While i < 4
                If b(i) = 0 Then      'b(i)等于零说明在这空白点的附近的空白点的附近同样没有同色棋子，故分数减20
                    b(i) = -20
                End If
                i = i + 1
            End While
